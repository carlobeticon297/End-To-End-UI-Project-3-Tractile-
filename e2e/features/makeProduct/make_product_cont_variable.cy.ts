...
describe(
  ...
  () => {
    ...
    it("Verify that a recipe can be set to the “Continuous Fixed” process type and that the change is reflected correctly.", () => {
      recipePayload.name = "Recipe Continues Variable";
      ...
      cy.seedProduct(forOutputItem).then(($catalogItemInput) => {
        recipePayload.outputs[0].correlationId = $catalogItemInput;
      });
      ...
      cy.wait(2000);
      cy.step("Set the Process Type to “Continuous Variable.");
      cy.get("[data-testid='select-processType']").select(
        "Continuous Variable"
      );
      cy.wait(1000);
      ...
      cy.step(
        "Verify that the “Continuous Variable label is displayed on the recipe card."
      );
      cy.get("[data-testid='Production']").click();
